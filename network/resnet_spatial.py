...
def resnet50(image_input, is_training, embedding_dim, scope='resnet_v2_50', before_pool=False):
    ...
    with arg_scope([layers_lib.conv2d], activation_fn=None, normalizer_fn=None, biases_initializer=None):
        spatial_out = layers_lib.conv2d(out, 1, 1, scope='spatial')
        spatial_weights = tf.nn.softmax(tf.reshape(spatial_out, (-1, out.shape[1] * out.shape[2])), axis=1)
        spatial_weights = tf.reshape(spatial_weights, (-1, out.shape[1], out.shape[2], 1))
        spatial_out = tf.reduce_sum(out * spatial_weights, axis=[1,2])

    fc_out = layers_lib.fully_connected(spatial_out, embedding_dim, activation_fn=None)

    if before_pool:
        return out, spatial_out, spatial_weights, fc_out
    else:
        return fc_out
